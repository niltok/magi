...
vector<vector<Bullets_Info>> info_stage {
    ...
    // 关卡2
    {
        Bullets_Info (1000,3,magi::Color(0,0,0),SMALL_,LOW_,Line),
        Bullets_Info (5000,3,c2,SMALL_,LOW_,Line),
        Bullets_Info (7000,3,c2,SMALL_,LOW_,Line),
    }
};
...
struct bullets_effect : public magi::Bullets {
    ...
    magi::Point operator[](size_t index) {
        long push = 0;
        while ( index > info_stage[Num][Before+push].n){ index = index - info_stage[Num][Before+push].n; push++;}
        switch (info_stage[Num][Before+push].bullets[index].kind){
        case Line:
            info_stage[Num][Before+push].bullets[index].Pos_Line();
            break;
        case Circle:
            info_stage[Num][Before+push].bullets[index].Pos_Circal();
            break;
        }
        return info_stage[Num][Before+push].bullets[index].point ;
